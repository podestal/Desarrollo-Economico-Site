...
<section className='sticky top-0 h-[100vh] w-full'>
        {loading && <img src={islaData.preImg} loading='lazy' role='presentation' className='h-screen object-cover  absolute top-0 right-0 w-full z-20' /> }
        <img src={islaData.img} onLoad={() => setLoading(false) }  loading='lazy' role='presentation' className='h-screen object-cover  absolute top-0 right-0 w-full' />
            ...
            <motion.div 
                ...
                className=" bg-black opacity-70 absolute top-0 right-0 w-full z-10 text-white h-screen overflow-scroll">
                    <motion.div 
                        ...
                        <h3 className='max-lg:text-6xl text-8xl mb-10 text-center'>{islaData.title}</h3>
                        <p className='max-lg:text-lg text-center text-2xl leading-[2.8rem] mx-2'>{islaData.content}</p>  
                        <p className='max-lg:text-lg text-center text-2xl leading-[2.8rem] mt-6 mx-2'>{islaData.contentS}</p>  
                    </motion.div>
            </motion.div>    
        </div>}
        </AnimatePresence>
